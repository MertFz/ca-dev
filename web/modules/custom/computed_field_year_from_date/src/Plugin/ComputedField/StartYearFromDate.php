...
namespace Drupal\computed_field_year_from_date\Plugin\ComputedField;
...
/**
 * Provides a computed field for the start year from a date field.
 */
#[ComputedField(
  id: 'computed_field_year_from_date_startjaar',
  label: new TranslatableMarkup('Startjaar'),
  field_type: 'string',
  cardinality: 1,
)]
class StartYearFromDate extends ComputedFieldBase {
  ...
  public function computeValue(EntityInterface $host_entity, ComputedFieldDefinitionWithValuePluginInterface $computed_field_definition): array {
    ...
    if ($host_entity->hasField('field_startdatum')) {
      $date_value = $host_entity->get('field_startdatum')->value;
      if ($date_value) {
